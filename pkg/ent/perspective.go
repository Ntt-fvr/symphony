// Copyright (c) 2004-present Facebook All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

// Code generated by entc, DO NOT EDIT.

package ent

import (
	"fmt"
	"strings"
	"time"

	"github.com/facebook/ent/dialect/sql"
	"github.com/facebookincubator/symphony/pkg/ent/perspective"
)

// Perspective is the model entity for the Perspective schema.
type Perspective struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// CreateTime holds the value of the "create_time" field.
	CreateTime time.Time `json:"create_time,omitempty"`
	// UpdateTime holds the value of the "update_time" field.
	UpdateTime time.Time `json:"update_time,omitempty"`
	// Name holds the value of the "name" field.
	Name string `json:"name,omitempty"`
	// Edges holds the relations/edges for other nodes in the graph.
	// The values are being populated by the PerspectiveQuery when eager-loading is set.
	Edges PerspectiveEdges `json:"edges"`
}

// PerspectiveEdges holds the relations/edges for other nodes in the graph.
type PerspectiveEdges struct {
	// PerspectiveFk holds the value of the perspectiveFk edge.
	PerspectiveFk []*Kqi
	// loadedTypes holds the information for reporting if a
	// type was loaded (or requested) in eager-loading or not.
	loadedTypes [1]bool
}

// PerspectiveFkOrErr returns the PerspectiveFk value or an error if the edge
// was not loaded in eager-loading.
func (e PerspectiveEdges) PerspectiveFkOrErr() ([]*Kqi, error) {
	if e.loadedTypes[0] {
		return e.PerspectiveFk, nil
	}
	return nil, &NotLoadedError{edge: "perspectiveFk"}
}

// scanValues returns the types for scanning values from sql.Rows.
func (*Perspective) scanValues() []interface{} {
	return []interface{}{
		&sql.NullInt64{},  // id
		&sql.NullTime{},   // create_time
		&sql.NullTime{},   // update_time
		&sql.NullString{}, // name
	}
}

// assignValues assigns the values that were returned from sql.Rows (after scanning)
// to the Perspective fields.
func (pe *Perspective) assignValues(values ...interface{}) error {
	if m, n := len(values), len(perspective.Columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	value, ok := values[0].(*sql.NullInt64)
	if !ok {
		return fmt.Errorf("unexpected type %T for field id", value)
	}
	pe.ID = int(value.Int64)
	values = values[1:]
	if value, ok := values[0].(*sql.NullTime); !ok {
		return fmt.Errorf("unexpected type %T for field create_time", values[0])
	} else if value.Valid {
		pe.CreateTime = value.Time
	}
	if value, ok := values[1].(*sql.NullTime); !ok {
		return fmt.Errorf("unexpected type %T for field update_time", values[1])
	} else if value.Valid {
		pe.UpdateTime = value.Time
	}
	if value, ok := values[2].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field name", values[2])
	} else if value.Valid {
		pe.Name = value.String
	}
	return nil
}

// QueryPerspectiveFk queries the perspectiveFk edge of the Perspective.
func (pe *Perspective) QueryPerspectiveFk() *KqiQuery {
	return (&PerspectiveClient{config: pe.config}).QueryPerspectiveFk(pe)
}

// Update returns a builder for updating this Perspective.
// Note that, you need to call Perspective.Unwrap() before calling this method, if this Perspective
// was returned from a transaction, and the transaction was committed or rolled back.
func (pe *Perspective) Update() *PerspectiveUpdateOne {
	return (&PerspectiveClient{config: pe.config}).UpdateOne(pe)
}

// Unwrap unwraps the entity that was returned from a transaction after it was closed,
// so that all next queries will be executed through the driver which created the transaction.
func (pe *Perspective) Unwrap() *Perspective {
	tx, ok := pe.config.driver.(*txDriver)
	if !ok {
		panic("ent: Perspective is not a transactional entity")
	}
	pe.config.driver = tx.drv
	return pe
}

// String implements the fmt.Stringer.
func (pe *Perspective) String() string {
	var builder strings.Builder
	builder.WriteString("Perspective(")
	builder.WriteString(fmt.Sprintf("id=%v", pe.ID))
	builder.WriteString(", create_time=")
	builder.WriteString(pe.CreateTime.Format(time.ANSIC))
	builder.WriteString(", update_time=")
	builder.WriteString(pe.UpdateTime.Format(time.ANSIC))
	builder.WriteString(", name=")
	builder.WriteString(pe.Name)
	builder.WriteByte(')')
	return builder.String()
}

// Perspectives is a parsable slice of Perspective.
type Perspectives []*Perspective

func (pe Perspectives) config(cfg config) {
	for _i := range pe {
		pe[_i].config = cfg
	}
}
