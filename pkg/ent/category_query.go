...
func (cq *CategoryQuery) QueryCategoryFk() *KqiQuery {
	query := &KqiQuery{config: cq.config}
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withCategoryFk: cq.withCategoryFk.Clone(),
...
func (cq *CategoryQuery) WithCategoryFk(opts ...func(*KqiQuery)) *CategoryQuery {
	query := &KqiQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCategoryFk = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withCategoryFk; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Category)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.CategoryFk = []*Kqi{}
		}
		query.withFKs = true
		query.Where(predicate.Kqi(func(s *sql.Selector) {
			s.Where(sql.InValues(category.CategoryFkColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.category_category_fk
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_category_fk" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_category_fk" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CategoryFk = append(node.Edges.CategoryFk, n)
		}
	}
