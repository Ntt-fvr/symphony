...
// QueryProjectValue chains the current query on the project_value edge.
func (pq *PropertyQuery) QueryProjectValue() *ProjectQuery {
	query := &ProjectQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *PropertyQuery) QueryPropertyValue() *PropertyValueQuery {
	query := &PropertyValueQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PropertyQuery) Clone() *PropertyQuery {
	...
	return &PropertyQuery{
		...
		withPropertyValue:      pq.withPropertyValue.Clone(),
...
//  WithProjectValue tells the query-builder to eager-loads the nodes that are connected to
// the "project_value" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PropertyQuery) WithProjectValue(opts ...func(*ProjectQuery)) *PropertyQuery {
	query := &ProjectQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withProjectValue = query
	return pq
}
...
func (pq *PropertyQuery) WithPropertyValue(opts ...func(*PropertyValueQuery)) *PropertyQuery {
	query := &PropertyValueQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPropertyValue = query
	return pq
}
...
func (pq *PropertyQuery) sqlAll(ctx context.Context) ([]*Property, error) {
	...
	if query := pq.withProjectValue; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Property)
		for i := range nodes {
			if fk := nodes[i].property_project_value; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "property_project_value" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withPropertyValue; query != nil {
		...
		query.Where(predicate.PropertyValue(func(s *sql.Selector) {
			s.Where(sql.InValues(property.PropertyValueColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.property_property_value
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "property_property_value" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "property_property_value" returned %v for node %v`, *fk, n.ID)
			}
