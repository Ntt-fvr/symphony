...
func (eq *EventQuery) QueryEventseverity() *EventSeverityQuery {
	query := &EventSeverityQuery{config: eq.config}
...
// QueryRuleEvent chains the current query on the ruleEvent edge.
func (eq *EventQuery) QueryRuleEvent() *RuleQuery {
	query := &RuleQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withEventseverity: eq.withEventseverity.Clone(),
		withRuleEvent:     eq.withRuleEvent.Clone(),
...
func (eq *EventQuery) WithEventseverity(opts ...func(*EventSeverityQuery)) *EventQuery {
	query := &EventSeverityQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withEventseverity = query
	return eq
}

//  WithRuleEvent tells the query-builder to eager-loads the nodes that are connected to
// the "ruleEvent" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EventQuery) WithRuleEvent(opts ...func(*RuleQuery)) *EventQuery {
	query := &RuleQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withRuleEvent = query
	return eq
}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	var (
		...
		_spec       = eq.querySpec()
		loadedTypes = [2]bool{
			eq.withEventseverity != nil,
			eq.withRuleEvent != nil,
		}
	)
	if eq.withEventseverity != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withEventseverity; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Event)
		for i := range nodes {
			if fk := nodes[i].event_severity_eventseverityevent; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_severity_eventseverityevent" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := eq.withRuleEvent; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Event)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.RuleEvent = []*Rule{}
		}
		query.withFKs = true
		query.Where(predicate.Rule(func(s *sql.Selector) {
			s.Where(sql.InValues(event.RuleEventColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.event_rule_event
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_rule_event" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_rule_event" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.RuleEvent = append(node.Edges.RuleEvent, n)
		}
	}
