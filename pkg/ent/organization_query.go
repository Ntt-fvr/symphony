...
func (oq *OrganizationQuery) QueryUserFk() *UserQuery {
...
// QueryWorkOrderFk chains the current query on the work_order_fk edge.
func (oq *OrganizationQuery) QueryWorkOrderFk() *WorkOrderQuery {
	query := &WorkOrderQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithUserFk(opts ...func(*UserQuery)) *OrganizationQuery {
	query := &UserQuery{config: oq.config}
...
//  WithWorkOrderFk tells the query-builder to eager-loads the nodes that are connected to
// the "work_order_fk" edge. The optional arguments used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithWorkOrderFk(opts ...func(*WorkOrderQuery)) *OrganizationQuery {
	query := &WorkOrderQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context) ([]*Organization, error) {
	...
	if query := oq.withUserFk; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Organization)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserFk = []*User{}
		}
		query.withFKs = true
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(organization.UserFkColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.organization_user_fk
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "organization_user_fk" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
	...
	if query := oq.withWorkOrderFk; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Organization)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.WorkOrderFk = []*WorkOrder{}
		}
		query.withFKs = true
		query.Where(predicate.WorkOrder(func(s *sql.Selector) {
			s.Where(sql.InValues(organization.WorkOrderFkColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.organization_work_order_fk
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "organization_work_order_fk" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
