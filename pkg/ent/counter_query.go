...
func (cq *CounterQuery) QueryCounterfamily() *CounterFamilyQuery {
	query := &CounterFamilyQuery{config: cq.config}
...
func (cq *CounterQuery) WithCounterfamily(opts ...func(*CounterFamilyQuery)) *CounterQuery {
	query := &CounterFamilyQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCounterfamily = query
	return cq
}
...
//		GroupBy(counter.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Counter.Query().
//		Select(counter.FieldCreateTime).
//		Scan(ctx, &v)
//
...
func (cq *CounterQuery) prepareQuery(ctx context.Context) error {
	...
	if err := counter.Policy.EvalQuery(ctx, cq); err != nil {
		return err
	}
	return nil
}
...
func (cq *CounterQuery) sqlAll(ctx context.Context) ([]*Counter, error) {
	...
	if query := cq.withCounterfamily; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Counter)
		for i := range nodes {
			if fk := nodes[i].counter_family_counterfamily; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(counterfamily.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "counter_family_counterfamily" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Counterfamily = n
			}
		}
	}
	...
	if query := cq.withCounterFk; query != nil {
		...
		for _, n := range neighbors {
			fk := n.counter_counter_fk
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "counter_counter_fk" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "counter_counter_fk" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.CounterFk = append(node.Edges.CounterFk, n)
		}
	}
