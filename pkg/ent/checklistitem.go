...
func (cli *CheckListItem) assignValues(values ...interface{}) error {
	...
	if value, ok := values[1].(*sql.NullString); !ok {
		...
		cli.Type = enum.CheckListItemType(value.String)
	}
	...
	if value, ok := values[7].(*sql.NullString); !ok {
		...
		*cli.EnumSelectionModeValue = enum.CheckListItemEnumSelectionMode(value.String)
	}
...
func (cli *CheckListItem) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", cli.Type))
