...
// PropertyValueEdge is the edge representation of PropertyValue.
type PropertyValueEdge struct {
	Node   *PropertyValue `json:"node"`
	Cursor Cursor         `json:"cursor"`
}

// PropertyValueConnection is the connection containing edges to PropertyValue.
type PropertyValueConnection struct {
	Edges      []*PropertyValueEdge `json:"edges"`
	PageInfo   PageInfo             `json:"pageInfo"`
	TotalCount int                  `json:"totalCount"`
}

// PropertyValuePaginateOption enables pagination customization.
type PropertyValuePaginateOption func(*propertyValuePager) error

// WithPropertyValueOrder configures pagination ordering.
func WithPropertyValueOrder(order *PropertyValueOrder) PropertyValuePaginateOption {
	if order == nil {
		order = DefaultPropertyValueOrder
	}
	o := *order
	return func(pager *propertyValuePager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultPropertyValueOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithPropertyValueFilter configures pagination filter.
func WithPropertyValueFilter(filter func(*PropertyValueQuery) (*PropertyValueQuery, error)) PropertyValuePaginateOption {
	return func(pager *propertyValuePager) error {
		if filter == nil {
			return errors.New("PropertyValueQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
func (p *propertyValuePager) toCursor(pv *PropertyValue) Cursor {
	return p.order.Field.toCursor(pv)
}

func (p *propertyValuePager) applyCursors(query *PropertyValueQuery, after, before *Cursor) *PropertyValueQuery {
	for _, predicate := range cursorsToPredicates(
		p.order.Direction, after, before,
		p.order.Field.field, DefaultPropertyValueOrder.Field.field,
	) {
		query = query.Where(predicate)
	}
	return query
...
func (p *propertyValuePager) applyOrder(query *PropertyValueQuery, reverse bool) *PropertyValueQuery {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	query = query.Order(direction.orderFunc(p.order.Field.field))
	if p.order.Field != DefaultPropertyValueOrder.Field {
		query = query.Order(direction.orderFunc(DefaultPropertyValueOrder.Field.field))
	}
	return query
}

// Paginate executes the query and returns a relay based cursor connection to PropertyValue.
func (pv *PropertyValueQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...PropertyValuePaginateOption,
) (*PropertyValueConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	...
	conn := &PropertyValueConnection{Edges: []*PropertyValueEdge{}}
	if !hasCollectedField(ctx, edgesField) ||
		first != nil && *first == 0 ||
		last != nil && *last == 0 {
		if hasCollectedField(ctx, totalCountField) ||
			hasCollectedField(ctx, pageInfoField) {
			count, err := pv.Count(ctx)
			if err != nil {
				return nil, err
			}
			conn.TotalCount = count
	...
	pv = pager.applyCursors(pv, after, before)
	pv = pager.applyOrder(pv, last != nil)
	var limit int
	if first != nil {
		limit = *first + 1
	} else if last != nil {
	...
	if field := getCollectedField(ctx, edgesField, nodeField); field != nil {
		pv = pv.collectField(graphql.GetOperationContext(ctx), *field)
	}
	...
	var nodeAt func(int) *PropertyValue
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *PropertyValue {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *PropertyValue {
	...
	conn.Edges = make([]*PropertyValueEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &PropertyValueEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
var (
	// PropertyValueOrderFieldName orders PropertyValue by name.
	PropertyValueOrderFieldName = &PropertyValueOrderField{
		field: propertyvalue.FieldName,
		toCursor: func(pv *PropertyValue) Cursor {
			return Cursor{
				ID:    pv.ID,
				Value: pv.Name,
			}
...
// String implement fmt.Stringer interface.
func (f PropertyValueOrderField) String() string {
	var str string
	switch f.field {
...
// MarshalGQL implements graphql.Marshaler interface.
func (f PropertyValueOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *PropertyValueOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("PropertyValueOrderField %T must be a string", v)
	}
	switch str {
	case "NAME":
		*f = *PropertyValueOrderFieldName
	default:
		return fmt.Errorf("%s is not a valid PropertyValueOrderField", str)
	}
	return nil
}

// PropertyValueOrderField defines the ordering field of PropertyValue.
type PropertyValueOrderField struct {
	field    string
	toCursor func(*PropertyValue) Cursor
}

// PropertyValueOrder defines the ordering of PropertyValue.
type PropertyValueOrder struct {
	Direction OrderDirection           `json:"direction"`
	Field     *PropertyValueOrderField `json:"field"`
}

// DefaultPropertyValueOrder is the default ordering of PropertyValue.
var DefaultPropertyValueOrder = &PropertyValueOrder{
	Direction: OrderDirectionAsc,
	Field: &PropertyValueOrderField{
		field: propertyvalue.FieldID,
		toCursor: func(pv *PropertyValue) Cursor {
