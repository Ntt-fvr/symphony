...
type ResourceQuery struct {
	...
	// eager-loading edges.
	withResourcespec *ResourceSpecificationQuery
	withResourceA    *ResourceRelationshipQuery
	withResourceB    *ResourceRelationshipQuery
	withFKs          bool
...
// QueryResourcespec chains the current query on the resourcespec edge.
func (rq *ResourceQuery) QueryResourcespec() *ResourceSpecificationQuery {
	query := &ResourceSpecificationQuery{config: rq.config}
...
// QueryResourceA chains the current query on the resource_a edge.
func (rq *ResourceQuery) QueryResourceA() *ResourceRelationshipQuery {
	query := &ResourceRelationshipQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryResourceB chains the current query on the resource_b edge.
func (rq *ResourceQuery) QueryResourceB() *ResourceRelationshipQuery {
	query := &ResourceRelationshipQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withResourcespec: rq.withResourcespec.Clone(),
		withResourceA:    rq.withResourceA.Clone(),
		withResourceB:    rq.withResourceB.Clone(),
...
//  WithResourcespec tells the query-builder to eager-loads the nodes that are connected to
// the "resourcespec" edge. The optional arguments used to configure the query builder of the edge.
func (rq *ResourceQuery) WithResourcespec(opts ...func(*ResourceSpecificationQuery)) *ResourceQuery {
	query := &ResourceSpecificationQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withResourcespec = query
	return rq
}

//  WithResourceA tells the query-builder to eager-loads the nodes that are connected to
// the "resource_a" edge. The optional arguments used to configure the query builder of the edge.
func (rq *ResourceQuery) WithResourceA(opts ...func(*ResourceRelationshipQuery)) *ResourceQuery {
	query := &ResourceRelationshipQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withResourceA = query
	return rq
}

//  WithResourceB tells the query-builder to eager-loads the nodes that are connected to
// the "resource_b" edge. The optional arguments used to configure the query builder of the edge.
func (rq *ResourceQuery) WithResourceB(opts ...func(*ResourceRelationshipQuery)) *ResourceQuery {
	query := &ResourceRelationshipQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withResourceB = query
	return rq
}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context) ([]*Resource, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [3]bool{
			rq.withResourcespec != nil,
			rq.withResourceA != nil,
			rq.withResourceB != nil,
		}
	)
	if rq.withResourcespec != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withResourcespec; query != nil {
	...
	if query := rq.withResourceA; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Resource)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ResourceA = []*ResourceRelationship{}
		}
		query.withFKs = true
		query.Where(predicate.ResourceRelationship(func(s *sql.Selector) {
			s.Where(sql.InValues(resource.ResourceAColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.resource_resource_a
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "resource_resource_a" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "resource_resource_a" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ResourceA = append(node.Edges.ResourceA, n)
		}
	}

	if query := rq.withResourceB; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Resource)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ResourceB = []*ResourceRelationship{}
		}
		query.withFKs = true
		query.Where(predicate.ResourceRelationship(func(s *sql.Selector) {
			s.Where(sql.InValues(resource.ResourceBColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.resource_resource_b
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "resource_resource_b" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "resource_resource_b" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ResourceB = append(node.Edges.ResourceB, n)
		}
	}
