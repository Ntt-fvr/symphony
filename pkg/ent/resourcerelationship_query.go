...
type ResourceRelationshipQuery struct {
	...
	// eager-loading edges.
	withResourcea        *ResourceQuery
	withResourceb        *ResourceQuery
	withResourcelocation *LocationQuery
	withFKs              bool
...
// QueryResourcea chains the current query on the resourcea edge.
func (rrq *ResourceRelationshipQuery) QueryResourcea() *ResourceQuery {
	query := &ResourceQuery{config: rrq.config}
...
// QueryResourceb chains the current query on the resourceb edge.
func (rrq *ResourceRelationshipQuery) QueryResourceb() *ResourceQuery {
	query := &ResourceQuery{config: rrq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rrq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryResourcelocation chains the current query on the resourcelocation edge.
func (rrq *ResourceRelationshipQuery) QueryResourcelocation() *LocationQuery {
	query := &LocationQuery{config: rrq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rrq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rrq *ResourceRelationshipQuery) Clone() *ResourceRelationshipQuery {
	...
	return &ResourceRelationshipQuery{
		...
		withResourcea:        rrq.withResourcea.Clone(),
		withResourceb:        rrq.withResourceb.Clone(),
		withResourcelocation: rrq.withResourcelocation.Clone(),
...
// the "resourcea" edge. The optional arguments used to configure the query builder of the edge.
func (rrq *ResourceRelationshipQuery) WithResourcea(opts ...func(*ResourceQuery)) *ResourceRelationshipQuery {
	query := &ResourceQuery{config: rrq.config}
	for _, opt := range opts {
		opt(query)
	}
	rrq.withResourcea = query
	return rrq
}

//  WithResourceb tells the query-builder to eager-loads the nodes that are connected to
// the "resourceb" edge. The optional arguments used to configure the query builder of the edge.
func (rrq *ResourceRelationshipQuery) WithResourceb(opts ...func(*ResourceQuery)) *ResourceRelationshipQuery {
	query := &ResourceQuery{config: rrq.config}
	for _, opt := range opts {
		opt(query)
	}
	rrq.withResourceb = query
	return rrq
}

//  WithResourcelocation tells the query-builder to eager-loads the nodes that are connected to
// the "resourcelocation" edge. The optional arguments used to configure the query builder of the edge.
func (rrq *ResourceRelationshipQuery) WithResourcelocation(opts ...func(*LocationQuery)) *ResourceRelationshipQuery {
	query := &LocationQuery{config: rrq.config}
	for _, opt := range opts {
		opt(query)
	}
	rrq.withResourcelocation = query
	return rrq
}
...
func (rrq *ResourceRelationshipQuery) sqlAll(ctx context.Context) ([]*ResourceRelationship, error) {
	var (
		...
		_spec       = rrq.querySpec()
		loadedTypes = [3]bool{
			rrq.withResourcea != nil,
			rrq.withResourceb != nil,
			rrq.withResourcelocation != nil,
		}
	)
	if rrq.withResourcea != nil || rrq.withResourceb != nil || rrq.withResourcelocation != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rrq.withResourcea; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*ResourceRelationship)
		for i := range nodes {
			if fk := nodes[i].resource_resource_a; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "resource_resource_a" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Resourcea = n
			}
		}
	}

	if query := rrq.withResourceb; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*ResourceRelationship)
		for i := range nodes {
			if fk := nodes[i].resource_resource_b; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "resource_resource_b" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Resourceb = n
			}
		}
	}

	if query := rrq.withResourcelocation; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*ResourceRelationship)
		for i := range nodes {
			if fk := nodes[i].location_rs_location; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "location_rs_location" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Resourcelocation = n
			}
		}
	}
