...
export const TYPE = 'TrueFalseBlock';
...
const defaultProperties = {
  attrs: {
    ...
    image: {
      ...VERTEX_COMMON_DISPLAY.defaultAttrProps,
      xlinkHref: '/inventory/static/svg/BlockFlowTrueFalse.svg',
      width: IMAGE_SIZE,
      height: IMAGE_SIZE,
      x: IMAGE_PADDING,
      y: IMAGE_PADDING,
...
defaultProperties.attrs.label.text = `${fbt('True / False', '')}`;
...
const TrueFalseBaseClass = jointJS.dia.Element.define(
  TYPE,
  defaultProperties,
  markup,
);
...
export function isTrueFalse(element: ?IVertexModel): boolean {
  if (element == null) {
    return false;
  }
