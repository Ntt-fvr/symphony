#!/usr/bin/env python3
# @generated AUTOGENERATED file. Do not Change!

from dataclasses import dataclass, field
from datetime import datetime
from gql_client.runtime.datetime_utils import DATETIME_FIELD_METADATA
from gql_client.runtime.variables import encode_variables
from gql import gql, Client
from gql.transport.exceptions import TransportQueryError
from functools import partial
from numbers import Number
from typing import Any, AsyncGenerator, Callable, Dict, List, Generator, Optional
from time import perf_counter
from dataclasses_json import DataClassJsonMixin

from ..input.survey_create_data import SurveyCreateData


# fmt: off
QUERY: List[str] = ["""
mutation CreateSurveyMutation($data: SurveyCreateData!) {
  createSurvey(data: $data)
}

"""
]


class CreateSurveyMutation:
    @dataclass(frozen=True)
    class CreateSurveyMutationData(DataClassJsonMixin):
        createSurvey: str

    # fmt: off
    @classmethod
    def execute(cls, client: Client, data: SurveyCreateData) -> str:
        variables: Dict[str, Any] = {"data": data}
        new_variables = encode_variables(variables)
        response_text = client.execute(
            gql("".join(set(QUERY))), variable_values=new_variables
        )
        res = cls.CreateSurveyMutationData.from_dict(response_text)
        return res.createSurvey

    # fmt: off
    @classmethod
    async def execute_async(cls, client: Client, data: SurveyCreateData) -> str:
        variables: Dict[str, Any] = {"data": data}
        new_variables = encode_variables(variables)
        response_text = await client.execute_async(
            gql("".join(set(QUERY))), variable_values=new_variables
        )
        res = cls.CreateSurveyMutationData.from_dict(response_text)
        return res.createSurvey
