...
func utilityFunc(ctx context.Context, r generated.ResolverRoot, logger log.Logger, tenant string) error {
	...
	logger.For(ctx).Info("project with no templates", zap.Int("count", len(projectIds)))
	for _, workOrderID := range projectIds {
		workOrderTypeID, err := client.ProjectType.Query().
			Where(projecttype.HasProjectsWith(project.ID(workOrderID))).
			OnlyID(ctx)
		if err != nil {
			return fmt.Errorf("failed to query project type: %w", err)
		}
		workOrderTemplate, typeToType, err := resolver.AddWorkOrderTemplate(ctx, client, workOrderTypeID)
		if err != nil {
			return fmt.Errorf("failed to create project template: %w", err)
		}
		err = client.Project.UpdateOneID(workOrderID).
			SetTemplate(workOrderTemplate).
			Exec(ctx)
		if err != nil {
